...
-----------------------------------------------*/
ENT.Model = {"models/cpthazama/zombiesurvival/wraith.mdl"} -- Leave empty if using more than one model
ENT.StartHealth = 175
...
ENT.NextTPT = CurTime() +math.Rand(5,10)
---------------------------------------------------------------------------------------------------------------------------------------------
...
function ENT:CustomOnThink()
	...
	if IsValid(self:GetEnemy()) then
		...
		if CurTime() > self.NextTPT && dist < 600 && self:Visible(enemy) then
			...
			self:SetPos(enemy:GetPos() +enemy:GetForward() *math.random(-100,-50))
			if !util.IsInWorld(self:GetPos()) then
				self:SetPos(oldPos)
				return
			end
			self:EmitSound("npc/stalker/breathing3.wav",80,100)
			util.ScreenShake(ent:GetPos(),16,100,3,50)
			local effectdata = EffectData()
