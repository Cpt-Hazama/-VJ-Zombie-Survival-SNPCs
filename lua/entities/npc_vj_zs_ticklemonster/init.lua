...
-----------------------------------------------*/
ENT.Model = {"models/player/zombie_fast.mdl"} -- Leave empty if using more than one model
...
function ENT:CustomOnInitialize()
	...
	self:AdjustBones(targetbones,Vector(10,0,0))
	self:AdjustBones({"ValveBiped.Bip01_Neck1"},Angle(0,70,0))
...
function ENT:CustomOnThink()
	...
	if self.MeleeAttacking then
		...
		self:AdjustBones({"ValveBiped.Bip01_L_Forearm","ValveBiped.Bip01_R_Forearm"},Vector(0,0,0))
		self:AdjustBones({"ValveBiped.Bip01_L_Upperarm", "ValveBiped.Bip01_R_Upperarm"},Vector(1,1,1))
	end
