...
local function CollideCallback(oldparticle, hitpos, hitnormal)
	...
	util.Decal("VJ_Blood_Red", pos, hitpos - hitnormal)
	...
	for i=1, num do
		local particle = emitter:Add("decals/blood"..math.random(6), pos)
...
function EFFECT:Init(data)
	...
	for i = 1, data:GetMagnitude() do
		local heading = (VectorRand():GetNormalized() * 8 +dir) / 4
		local particle = emitter:Add("decals/blood"..math.random(6), pos + heading)
